...
Cities = 100
FinalSteps = 250
CitySizeRange = [500, 10_000] # population size per city
...
function create_params(;
    ...
    Random.seed!(seed)
    Ns = rand(CitySizeRange[1]:CitySizeRange[2], C)
...
params = create_params(C = Cities, max_travel_rate = 0.01)
...
record(fig, "covid_evolution.mp4"; framerate = 5) do io
    for j in 1:VideoSteps
        model.events = 0
        Agents.step!(abmobs, 1)
        println("Step $j / $VideoSteps — events this step: $(model.events)")
        recordframe!(io)
    end
    recordframe!(io)
end
...
data, _ = run!(model, FinalSteps; adata = to_collect)
data[1:10, :]
